...
function logMessage(message) {
  ...
  let main = async () => {
    try {
      ...
      logMessage("Count Neighbors Kernel executed.");
      ...
      logMessage("Update Liveness Kernel executed.");
      ...
      const transferLiveCells = ti.kernel(() => {
        ...
        transferIdx[0] = idx; // Capture idx for debugging
      ...
      logMessage("Transfer Live Cells Kernel executed.");
      ...
      async function frame() {
        try {
          ...
          // Retrieve and log the live cell count and idx
          const liveCellCountArray = await liveCellCount.toArray();
          const transferIdxArray = await transferIdx.toArray();
          logMessage(`Frame ${frameCount}: Live cell count = ${liveCellCountArray[0]}, idx = ${transferIdxArray[0]}`);
